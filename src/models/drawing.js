...
const QUERY = {
    ...
    get: {
        perIsometric: `SELECT A.${table.COLUMN.NAME} AS FILE_NAME, A.${table.COLUMN.VERSION} AS VERSION, B.${isometricTable.COLUMN.ISO_NO} AS ISO_NO FROM ${table.TABLE} AS A JOIN ${isometricTable.TABLE} AS B ON A.${table.COLUMN.ISOMETRIC_ID} = B.${isometricTable.COLUMN.ID} WHERE B.${isometricTable.COLUMN.ID} = ?`,
        onlyOne: `SELECT A.${table.COLUMN.NAME} AS FILE_NAME, A.${table.COLUMN.VERSION} AS VERSION, B.${isometricTable.COLUMN.ISO_NO} AS ISO_NO FROM ${table.TABLE} AS A JOIN ${isometricTable.TABLE} AS B ON A.${table.COLUMN.ISOMETRIC_ID} = B.${isometricTable.COLUMN.ID} WHERE A.${table.COLUMN.ID} = ?`,
    },
    insert: {
